...
Recall from lab1 that there are 2 items (we know this from the totalItems value of 2, which represents the "ansible1" and "curl1") and that each item represents a policy.
...
We have told jq to only display collections wihtin the items values, specifically we are specifying the first one, which again, is the first ASM policy.
